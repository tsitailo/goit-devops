...
- **Prometheus + Grafana** — моніторинг кластера та застосунку; Grafana підключена до Prometheus автоматично
...
│       ├── monitoring.tf        # Helm releases Prometheus та Grafana
...
Grafana credentials: `admin` / значення змінної `grafana_admin_password` (default: `admin123`).
Джерело даних Prometheus підключається автоматично при розгортанні.

Отримати пароль з K8s Secret:
```bash
...
| Змінна | Опис | Default |
|--------|------|---------|
| `monitoring_namespace` | Kubernetes namespace для Prometheus та Grafana | `monitoring` |
| `prometheus_chart_version` | Версія Helm chart `prometheus-community/prometheus` | `25.21.0` |
| `grafana_chart_version` | Версія Helm chart `grafana/grafana` | `7.3.11` |
| `grafana_admin_password` | Пароль адміністратора Grafana | `admin123`, sensitive |
| `prometheus_retention` | Час зберігання метрик Prometheus | `15d` |
| `prometheus_storage_size` | Розмір PersistentVolume для Prometheus | `10Gi` |
| `grafana_storage_size` | Розмір PersistentVolume для Grafana | `5Gi` |
